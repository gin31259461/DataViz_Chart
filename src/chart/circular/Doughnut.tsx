...
export default function Doughnut(props: ChartStyle & PieProps) {
	const svgRef = React.useRef<SVGSVGElement>(null);

	const handleLoad = () => {
		RemoveChart(svgRef);
		createDoughnut(svgRef, props);
	};
...
function createDoughnut(element: React.RefObject<SVGElement>, props: ChartStyle & PieProps) {
	let { data, mapper, base, margin, tooltip, animation, legend, font, pie, stroke, fill } = props;
	...
	if (pie.radius.outer === undefined)
		pie.radius.outer = Math.max(base.width - margin.left - margin.right, base.height - margin.top - margin.bottom) / 4;
	if (pie.radius.inner === undefined) pie.radius.inner = pie.radius.outer / 2;
	...
	const I = d3.range(label.length).filter((i) => !isNaN(value[i]) && distinctLabel.has(label[i]));
	...
	const arcLabel = d3.arc<d3.PieArcDatum<number>>().innerRadius(pie.radius.label).outerRadius(pie.radius.label);
	const arcValue = d3
		.arc<d3.PieArcDatum<number>>()
	...
	const pieLabel = svg.append('g').attr('text-anchor', 'middle').attr('transform', `translate(${pieCentroid})`);
	const Pie = svg.append('g').attr('transform', `translate(${pieCentroid})`);
	const pieValue = svg.append('g').attr('text-anchor', 'middle').attr('transform', `translate(${pieCentroid})`);
	...
	if (pie.value.enabled) {
		...
		pieValue
			...
			.data((d) => {
				...
				return d.endAngle - d.startAngle > (Number(font.size.slice(0, -2)) * 4) / 100 ? lines : '';
			})
			.join('tspan')
			.attr('x', 0)
			.attr('y', (_, i) => `${i * 1.1}em`)
	...
	if (animation.enabled) {
		Pie.selectAll('path')
			...
			.transition()
			.attrTween('d', (i) => {
				const interpolate = d3.interpolate({ startAngle: 0, endAngle: 0 }, divData[i]);
				return (t: number) => {
	...
	if (legend.enabled) {
		createLegend(svg, Array.from(distinctLabel), colorScale, props, onHover, noHover);
	}

	if (tooltip.enabled) {
		const { showTooltip, moveTooltip, hideTooltip } = createTooltip(svg, props, pieCentroid[0], pieCentroid[1]);
		Pie.selectAll('path')
			.data(I)
