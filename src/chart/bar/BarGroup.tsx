...
import { NumberValue, easeExpInOut, easeExpOut } from "d3";
...
function CreateBarGroup(
	...
	if (yAxis.domain === undefined) {
		const domain2 = d3.max(groupedData, (obj) =>
			d3.max(obj.value, (obj) => obj.y * 1.1)
		);
	...
	if (xAxis.enabled) {
		createXAxis(
			...
			props,
			false,
			barWidth * x.length * keys.length + barGap * x.length * (keys.length - 1),
			x
		);
	}
	...
	const offset =
		(groupedData.length * barWidth + (groupedData.length - 1) * barGap) / 2;
